...
\begin{abstract}
We study online control of a scalar system under adversarial disturbances, convex
cost functions, and state-dependent action constraints. At each time, the controller
must choose a feasible action before observing the corresponding disturbance and
cost. We introduce \emph{Simplex Disturbance-Action Control} (SDAC), a
simplex-constrained disturbance-action policy class that preserves per-step
feasibility by construction. An online controller updates SDAC parameters via
exponentiated gradient on the sub-probability simplex, with a feasibility projection
that enforces the state-dependent action constraint. We prove regret that is
sublinear in the horizon and logarithmic in the policy dimension against the best
fixed SDAC policy in hindsight, implying no-regret relative to feasible linear
state-feedback policies. The setting is motivated by energy-harvesting battery
management and similar storage systems.
\end{abstract}
...
corresponding disturbance and cost.
...
$1-\alpha$ of the state is lost each period). After the action is chosen, an
adversarial disturbance $w_t$ arrives, and the state evolves according to
\[
x_{t+1} = \alpha x_t - u_t + w_t.
\]

Unlike stochastic application models that rely on distributional assumptions, our
formulation is adversarial. Disturbances $w_t$ and costs $c_t(x, u)$ may vary
...
\begin{itemize}
    ...
    \item \textbf{Queueing Networks and Buffer Management:} $x_t$ is queue/buffer
        occupancy (retention factor $\alpha$ after abandonment/leakage), $w_t$ is
...
%The queueing-network literature has extensively studied systems with arrivals,
%storage (queues), and controlled service/allocations. A classical line of work
%focuses on \emph{adversarial queueing theory} (AQT), where arrivals and routing
%decisions may be chosen adversarially and the primary objective is to establish
%stability (i.e., bounded queue lengths) under appropriate load conditions
%\cite{borodin2001adversarial, gamarnik1998stability, kiwi2005survey}. While this
%model shares structural similarities with our setting in allowing worst-case inputs,
%its scope is fundamentally different: the goal is stability rather than optimization
%of a cost function, and no notion of regret or comparison to an optimal dynamic
%policy is considered. Moreover, in many classical adversarial queueing formulations,
%actions are \emph{discrete} (e.g., routing or scheduling decisions), whereas our
%setting considers \emph{continuous allocation decisions}.

%A second major line is \emph{stochastic network optimization} based on
%Lyapunov-drift arguments \cite{neely2010stochastic, georgiadis2006resource}. These
%works assume stochastic arrivals and network conditions and design policies (e.g.,
%max-weight or drift-plus-penalty) that achieve near-optimal time-average utility
%while ensuring queue stability. In these formulations, the stage utility
%(equivalently, negative cost) is modeled primarily as a function of the current
%control action, with queue states entering through stability constraints and drift
%terms rather than through a joint state-action adversarial loss. In addition, many
%formulations use discrete or finitely parameterized action sets (e.g., selecting
%from a finite set of rates or schedules), in contrast to our continuous decision
%space.

%A related direction is online convex optimization with long-term constraints using
%virtual queues to handle long-term constraints \cite{mahdavi2012trading,
%jenatton2016adaptive, yu2017online, JMLR:v21:16-494}. These works allow adversarial
%convex costs and provide regret and long-term constraint violation guarantees.
%However, the queueing structure is typically auxiliary, serving to enforce
%constraints rather than modeling a system's dynamic, and the objective does not
%explicitly depend on the evolving queue state. Furthermore, the benchmarks are
%standard regret notions relative to fixed comparators, rather than policies that
%account for state evolution.
...
disturbances (not adversarial), and benchmark against fixed allocation amounts rather
than fixed policies. In contrast, our framework jointly optimizes over state-action
costs, adversarial disturbances, and stronger benchmarks, requiring new
...
model permits fully adversarial sequences of disturbances and cost functions and allows
...
exogenous-disturbance model. The queueing literature provides useful motivation and
...
controller chooses the supply (equivalent to disturbances in our model). The
...
control action and observes incoming supply (disturbances). Methodologically, our
...
dynamical system is operated under adversarial disturbances and costs. The objective
...
policy parameterization in which control actions are expressed as affine or linear
functions of the current state and a finite history of past disturbances. This
...
arbitrary time-varying disturbances and unknown general convex cost functions, and the
...
\begin{itemize}
    ...
    \item \textbf{Algorithmic and analytical contribution.} We develop an
        exponentiated-gradient online controller over SDAC parameters in which a
        feasibility projection \emph{enforces} the state-dependent constraint on the
        online control action, and we analyze regret against the best fixed SDAC
        policy in hindsight, achieving sublinear regret with logarithmic dependence
        on policy dimension.
\end{itemize}
...
guarantees, and Section~\ref{sec:conclusion} concludes. Deferred proofs appear in the
Appendix.
...
We consider a discrete-time process over times $t\in\{1,2,\ldots,T\}$. The system
state is denoted by $x_t\in\mathbb{R}_{\ge 0}$, where $x_t$ represents the resource
level at the beginning of time $t$. The initial state is given as
\[
x_1 = 0.
\]
...
controller chooses $u_t$ before observing $w_t$ and $c_t$. Hence both the disturbance
...
\begin{enumerate}
    ...
    \item \textbf{Adversarial timing:} at time $t$, $u_t$ is chosen before observing
        $w_t$ and $c_t$.
    ...
    \item \textbf{Feasible policy class:} both the online policy to be developed and the benchmark
     described below are required to satisfy \eqref{eq:model-feasibility} under dynamics
        \eqref{eq:model-dynamics}.
\end{enumerate}

For an online algorithm $\mathcal{A}$ producing trajectory
$\{(x_t^{\mathcal{A}},u_t^{\mathcal{A}})\}_{t=1}^T$, define cumulative cost
\begin{equation}
\label{eq:def-cumulative-cost}
J_T(\mathcal{A}) \triangleq \sum_{t=1}^T c_t\bigl(x_t^{\mathcal{A}},u_t^{\mathcal{A}}\bigr).
\end{equation}
For a fixed positive integer $H$, let the policy class be $\Pi\triangleq\mathrm{SDAC}_H$ (defined in
Section~\ref{sec:sdac}). For each $\pi\in\Pi$, let
$\{(x_t^{\pi},u_t^{\pi})\}_{t=1}^T$ be the induced feasible trajectory under the same
disturbance sequence. Regret is defined by
\begin{equation}
\label{eq:def-regret}
\mathrm{Regret}(T)
\triangleq
...
\eqref{eq:model-feasibility} for all $t$ and achieves sublinear regret as defined in
\eqref{eq:def-regret}.
...
function of a finite window of past disturbances. This representation is central in
...
\qquad x_1^{\pi(M)}=0.
\end{equation}
The weights $\alpha^i$ align the disturbance filter with the retention factor
$\alpha$ in \eqref{eq:model-dynamics}.


First, feasibility is preserved within the class: for any fixed $M\in\Delta_H$, the
induced trajectory under \eqref{eq:def-sdac-policy}--\eqref{eq:sdac-state-update}
satisfies $0\le u_t^{\pi(M)}\le \alpha x_t^{\pi(M)}$ for all $t$ (see
Lemma~\ref{lem:appendix-feasibility}).

Second, SDAC is compatible with online convex optimization. For fixed disturbance
sequences, $u_t^{\pi(M)}$ is linear in $M$ and $x_t^{\pi(M)}$ is affine in $M$ (see
Lemma~\ref{lem:appendix-state-decomp}). Therefore, since each $c_t$ is convex in
$(x,u)$, the induced cost is convex in $M$ (see Lemma~\ref{lem:appendix-surrogate}).
...
control-action, state, and cost errors that decay geometrically in $H$.
...
\qquad i=1,\ldots,H.
\]
Then $M_q^\star\in\Delta_H$ with $\sum_{i=1}^H M_q^{\star(i)}=1-(\beta/\alpha)^H$,
and for every $\{w_t\}\subset[0,1]$ and all $t\ge 1$,
\begin{equation}
...
\end{aligned}
\end{equation}
In particular, the errors decay geometrically in $H$, so $\mathrm{SDAC}_H$
...
unnormalized-negentropy online mirror descent (equivalently, exponentiated gradient)
over the sub-probability simplex $\Delta_H\subset\mathbb{R}^H$ with a fixed step size
$\eta>0$.

For each time $t$, let $M_t\in\Delta_H$ be the online parameter vector. We initialize
at
\begin{equation}
\label{eq:alg-init}
M_1^{(i)}\triangleq\frac{1}{H+1},\qquad i=1,\ldots,H
\end{equation}
so that the unnormalized-negentropy Bregman diameter of $\Delta_H$ relative to $M_1$
is at most $\log(H+1)$ (see the proof of Theorem~\ref{thm:regret-bound}).
...
We verify that the algorithm satisfies \eqref{eq:model-feasibility} at every time.
The key mechanism is the feasibility projection in \eqref{eq:alg-feasible-action},
which maps the unconstrained control action onto the currently available feasible
interval $[0,\alpha x_t^{\mathcal{A}}]$. Since $M_t\in\Delta_H$ and
$\widetilde{w}_t\ge 0$ componentwise, equation \eqref{eq:alg-unconstrained-action}
gives $\widehat{u}_t\ge 0$. By \eqref{eq:alg-feasible-action},
\[
u_t^{\mathcal{A}}=\min\{\widehat{u}_t,\alpha x_t^{\mathcal{A}}\}\in[0,\alpha x_t^{\mathcal{A}}],
\]
so the control-action feasibility constraint holds whenever $x_t^{\mathcal{A}}\ge 0$.
It remains to prove $x_t^{\mathcal{A}}\ge 0$ for all $t$. The base case is
$x_1^{\mathcal{A}}=0$. If $x_t^{\mathcal{A}}\ge 0$, then $u_t^{\mathcal{A}}\le \alpha
x_t^{\mathcal{A}}$ and $w_t\in[0,1]$, so from \eqref{eq:alg-state-update},
\[
x_{t+1}^{\mathcal{A}}=\alpha x_t^{\mathcal{A}}-u_t^{\mathcal{A}}+w_t\ge 0.
\]
Thus, by induction, $x_t^{\mathcal{A}}\ge 0$ and $0\le u_t^{\mathcal{A}}\le \alpha
x_t^{\mathcal{A}}$ for all $t$.
...
$g_t\in\partial \ell_t(M_t)$ be a subgradient.
...
\end{equation}
followed by the closed-form Bregman projection onto $\Delta_H$\footnote{This step is
exactly the Bregman (generalized-KL) projection of $\widetilde{M}_{t+1}$ onto
$\Delta_H=\{M\ge0:\sum_i M^{(i)}\le1\}$ induced by the unnormalized negentropy;
renormalization is triggered only when the unprojected mirror step leaves $\Delta_H$,
i.e.\ when $\sum_j\widetilde{M}_{t+1}^{(j)}>1$. See Lemma~\ref{lem:eg-projection}.}:
\begin{equation}
...
\eqref{eq:alg-feasible-action} and performs online convex optimization over
$\Delta_H$ through \eqref{eq:alg-surrogate-cost}--\eqref{eq:alg-eg-normalized}.
...
By Lemma~\ref{lem:appendix-surrogate}, the surrogate costs are convex and
\(L\)-Lipschitz. Unnormalized-negentropy OMD on $\Delta_H$, initialized as in
\eqref{eq:alg-init}, yields Bregman diameter at most $\log(H+1)$, so the OMD term is
bounded by \(\log(H+1)/\eta + \eta L^2T/2\).
The remaining stability term is controlled by
Lemmas~\ref{lem:omd-move}--\ref{lem:stab-sum}, which together contribute at most
\(\tfrac{\alpha}{(1-\alpha)^2}\,\eta L^2T\). Combining the two bounds yields the
claim.
\end{proof}
...
Run the exponentiated-gradient controller with feasibility projection of
...
policy $\pi_q^{\mathrm{lin}}$ from Lemma~\ref{lem:sdac-linear-approx}. Then
\[
\begin{aligned}
&\sum_{t=1}^T c_t\bigl(x_t^{\mathcal{A}},u_t^{\mathcal{A}}\bigr)
...
\end{aligned}
\]
Thus the controller is no-regret against every fixed linear state-feedback policy
$u_t=q\,x_t$, $q\in(0,\alpha)$.
\end{corollary}
...
\end{aligned}
\]
Chaining the two displays gives the stated bound. For the asymptotic statement,
...
feasibility by construction, and the exponentiated-gradient controller with
...
This appendix collects the deferred proofs for structural SDAC properties used in
Section~\ref{sec:sdac} and the regret bound stated in
...
loop $x_{t+1}^{\mathrm{lin}}=\alpha x_t^{\mathrm{lin}}-q x_t^{\mathrm{lin}}+w_t=\beta
x_t^{\mathrm{lin}}+w_t$ with $x_1^{\mathrm{lin}}=0$. Since $\beta>0$ and $w_t\ge 0$,
...
closed-loop recursion from $x_1^{\mathrm{lin}}=0$,
\[
...
\]
Because $w_{t-i}=0$ for $i\ge t$, this equals
$q\sum_{i=1}^{\min\{H,t-1\}}\beta^{i-1}w_{t-i}$, so subtracting it from
$u_t^{\mathrm{lin}}$ cancels the first $\min\{H,t-1\}$ terms:
\[
...
the same disturbance, so the disturbance cancels on subtraction:
\[
...
\eqref{eq:sdac-state-update}. Then for all $t\ge 2$,
\begin{equation}
...
(release nothing, $x_{\tau+1}=\alpha x_\tau+w_\tau$), and the coefficients satisfy
$0\le \psi_t^{(i)}\le \rho_t\le 1/(1-\alpha)$. Consequently, setting
$M^{(0)}\triangleq 1-W\ge 0$, the identity \eqref{eq:appendix-state-decomp} writes
$x_t^{\pi(M)}$ as the convex combination $M^{(0)}\rho_t+\sum_{i=1}^H
M^{(i)}\psi_t^{(i)}$ of quantities in $[0,1/(1-\alpha)]$; in particular $M\mapsto
x_t^{\pi(M)}$ is affine and
\[
...
\begin{proof}
Recall $w_\tau\equiv 0$ for $\tau\le 0$. Isolating the $j=1$ term in the definition
...
\emph{Inductive step.} Assume \eqref{eq:appendix-state-decomp} holds at time $t\ge
2$. Using \eqref{eq:def-sdac-policy}, \eqref{eq:sdac-state-update}, and the induction
hypothesis,
\[
\begin{aligned}
x_{t+1}^{\pi(M)}
...
With $M^{(0)}=1-W\ge 0$, the parameters $M^{(0)},M^{(1)},\ldots,M^{(H)}$ are
nonnegative and sum to one, so \eqref{eq:appendix-state-decomp} exhibits
$x_t^{\pi(M)}$ as a convex combination of
$\rho_t,\psi_t^{(1)},\ldots,\psi_t^{(H)}\in[0,1/(1-\alpha)]$, giving $0\le
x_t^{\pi(M)}\le 1/(1-\alpha)$. Affineness in $M$ is immediate, as $\psi_t$ and
$\rho_t$ do not depend on $M$.
\end{proof}
...
u_t^{\pi(M)} = \langle M,\widetilde{w}_t\rangle,
\]
where $\widetilde{w}_t$ is the retention-weighted past-disturbance vector from
\eqref{eq:def-sdac-policy-vector}, with components
\[
...
\]
The lower bound is immediate from $w_{t-i}\ge 0$. For the upper bound, use
$w_\tau\in[0,1]$:
\[
...
Now fix $t\ge 2$. Lemma~\ref{lem:appendix-state-decomp} gives $x_t^{\pi(M)}=\langle
...
(Lemmas~\ref{lem:appendix-state-decomp}--\ref{lem:appendix-action-linear}) and $c_t$
is convex, so $\ell_t$ is convex.
...
1/(1-\alpha)$, and Lemma~\ref{lem:appendix-action-linear} gives
$\|\widetilde{w}_t\|_\infty\le \alpha$. This yields the claim, and one may take the
...
Let $\Psi(M)=\sum_{i=1}^H\bigl(M^{(i)}\log M^{(i)}-M^{(i)}\bigr)$ denote the
unnormalized negentropy on $\mathbb{R}_{\ge 0}^H$ (with the convention $0\log 0=0$),
and let
\[
D_\Psi(M\|V)=\sum_{i=1}^H\Bigl(M^{(i)}\log\tfrac{M^{(i)}}{V^{(i)}}-M^{(i)}+V^{(i)}\Bigr)
\]
be its Bregman divergence, the generalized Kullback--Leibler divergence. Fix
$\widetilde{M}\in\mathbb{R}_{>0}^H$ and set $S\triangleq\sum_{i=1}^H
\widetilde{M}^{(i)}$. Then the Bregman projection of $\widetilde{M}$ onto
$\Delta_H=\{M\in\mathbb{R}_{\ge0}^H:\sum_{i} M^{(i)}\le1\}$,
\[
...
\]
Consequently, taking $\widetilde{M}=\widetilde{M}_{t+1}$ from
\eqref{eq:alg-eg-unnormalized}, the update \eqref{eq:alg-eg-normalized} is exactly
the projection $M_{t+1}=P(\widetilde{M}_{t+1})$, and hence $M_{t+1}\in\Delta_H$.
\end{lemma}
...
\label{lem:omd-move}
For the unnormalized-negentropy OMD update with step size \(\eta>0\) and subgradient
...
\begin{proof}
Let \(\Psi(M)=\sum_{i=1}^H\bigl(M^{(i)}\log M^{(i)}-M^{(i)}\bigr)\). By
Lemma~\ref{lem:eg-projection}, \(M_{t+1}\) is the Bregman projection of the mirror
step \(\widetilde{M}_{t+1}\) onto \(\Delta_H\), so the first order optimality
condition gives, for all \(M\in\Delta_H\),
\[ 
\begin{aligned}
...
usual Pinsker-type bound for the generalized KL extends to the sub-probability
simplex \(\Delta_H\),
\begin{align*}
...
\end{equation}
For any fixed $M\in\Delta_H$ and every $t\ge 1$ the following two-sided recursion
holds:
\begin{align}
\Gamma_{t+1}^M
...
\eqref{eq:alg-state-update} (for the algorithm), the  disturbance $w_t$ cancels and
...
\begin{proof}
From \eqref{eq:GAMMA-Upper}, for any fixed benchmark $M\in\Delta_H$ and every $s\ge
1$,
\[
\Gamma_{s+1}^{M}
\le
...
\alpha\,[\Gamma_t^{M_t}]_+ ,
\]
which is the middle inequality. Finally, by Lemma~\ref{lem:state-gap-bound},
...
We bound the OMD term by unnormalized-negentropy OMD on
$\Delta_H\subset\mathbb{R}^H$. Let $\Psi(M)=\sum_{i=1}^H\bigl(M^{(i)}\log
M^{(i)}-M^{(i)}\bigr)$ and let $D_\Psi$ be its Bregman divergence as in
Lemma~\ref{lem:eg-projection}. Under the initialization \eqref{eq:alg-init}, for any
