...
\newcommand{\RegSDAC}[1]{\mathrm{Regret}_{\mathrm{SDAC}}(#1)}
\newcommand{\RegLin}[1]{\mathrm{Regret}_{\mathrm{lin}}(#1)}
...
that enforces the state-dependent action constraint. We prove that regret against the best fixed SDAC policy in hindsight is sublinear in
the horizon and logarithmic in the policy dimension, implying no-regret relative to
feasible linear state-feedback policies. The setting is motivated by energy-harvesting battery
...
feasible trajectory under the same disturbance sequence. Regret against the best
...
\paragraph{Surrogate cost and mirror-descent step}
...
We define the online-mirror-descent update in the compact, one-line form
\begin{equation}
...
\eqref{eq:alg-omd-argmin} is exactly the result of two elementary steps: an
unconstrained exponentiated-gradient step,
\begin{equation}
\label{eq:alg-eg-unnormalized}
\widetilde{M}_{t+1}^{(i)} \triangleq M_t^{(i)}\exp\!\bigl(-\eta g_t^{(i)}\bigr), \qquad i=1,\ldots,H,
\end{equation}
followed by renormalization, triggered only when the resulting mass exceeds one:
\begin{equation}
...
\eqref{eq:alg-feasible-action} and performs online mirror descent over $\Delta_H$
...
\begin{lemma}[Closed-form solution of the one-line OMD update]
\label{lem:omd-closed-form}
Fix $M_t\in\Delta_H$, $\eta>0$, and $g_t\in\mathbb{R}^H$, and let
$\widetilde{M}_{t+1}$ be defined coordinatewise by \eqref{eq:alg-eg-unnormalized},
...
\end{aligned}
\]
where $P(\cdot)$ is the Bregman projection of Lemma~\ref{lem:eg-projection}.
Consequently, the one-line update \eqref{eq:alg-omd-argmin} coincides exactly with
the two-step procedure \eqref{eq:alg-eg-unnormalized}--\eqref{eq:alg-eg-normalized},
...
$P(\widetilde{M}_{t+1})$. Lemma~\ref{lem:eg-projection} identifies
...
We bound the OMD term using the mirror-descent update \eqref{eq:alg-omd-argmin} with
