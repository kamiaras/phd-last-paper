...
$\alpha=1$ (no abandonment/leakage), so those settings involve related dynamics but are not
direct instances of the $\alpha\in(0,1)$ model studied in this paper. The case
$\alpha\ge 1$ can be handled by a straightforward extension (Section~\ref{sec:extensions});
we do not develop it here, as it adds technical overhead without
...
sum to one (or at most one, for SDAC). However, the controller in
...
approaches operating through nonlinear and nondifferentiable dynamics such as
...
\begin{itemize}
    ...
    \item \textbf{Algorithmic and analytical contribution.} We develop an
        online mirror descent (OMD) controller  over
        SDAC parameters that, via feasibility projection, 
...
x_1 = 0.
\end{equation}
Fix a state-retention factor $\alpha\in(0,1)$. At each time $t$, the controller chooses a
...
\begin{enumerate}
    ...
    \item \textbf{Feasible policy class:} both the online policy to be developed and the benchmark
     policies described below are required to satisfy \eqref{eq:model-feasibility} under the dynamics
...
adversarial online control because the cost is convex in terms of the parameters.
This representation yields robust regret guarantees and enables efficient online
convex-optimization updates. In our setting, a representative DAC form is
\begin{equation}
\label{eq:dac-param}
...
unconstrained DAC filters with a simplex-constrained family tailored to these dynamics.
...
\ell_t(M) \triangleq c_t\bigl(x_t^{\pi(M)},u_t^{\pi(M)}\bigr), \qquad M\in\Delta_H,
\end{equation}
which is convex and Lipschitz by Lemma~\ref{lem:sdac-surrogate}. Let
...
The algorithm yields a feasible online trajectory that satisfies
\eqref{eq:model-feasibility} via \eqref{eq:alg-feasible-action}, and performs OMD
over $\Delta_H$ through \eqref{eq:alg-surrogate-cost} and \eqref{eq:alg-omd-argmin}.
...
with the best fixed SDAC policy but also with the entire family of feasible linear
state-feedback policies.

\begin{corollary}[No-regret against linear state-feedback policies]
\label{cor:regret-vs-linear}
Run the OMD controller with feasibility projection from
Section~\ref{sec:online-algorithm}, using the step size $\eta^*$ of
...
\begin{proof}
By the definition of regret~\eqref{eq:def-regret} and the optimized bound of
...
&\quad +2L\sqrt{\Bigl(\tfrac{1}{2}+\tfrac{\alpha}{(1-\alpha)^2}\Bigr)T\log(H+1)}.
\end{aligned}
\]
...
Extending SDAC, our algorithm, and our analysis to handle $\alpha\ge 1$ is
...
environment for elegance and to avoid additional lengthy derivations.
...
$(x,u)$. The SDAC construction lifts by replacing the filter vector $M\in\Delta_H$ with a matrix
...
storage systems (non-diagonal $A$) fall outside this easy reduction.

These directions preserve feasibility by construction and retain the OMD structure
developed above; fleshing out the corresponding regret bounds is left for future work.
...
For the lower bound, Lemma~\ref{lem:sdac-feasibility} applied to the benchmark
...
$L$-Lipschitz by Lemma~\ref{lem:sdac-surrogate}, so the standard OMD regret bound
