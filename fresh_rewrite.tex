...
online mirror descent (OMD) on the sub-probability simplex, with a feasibility projection
...
sequences, $u_t^{\pi(M)}$ is linear in $M$ by
\eqref{eq:def-sdac-policy-vector}, and $x_t^{\pi(M)}$ is affine in $M$ (see
...
For each time $t$, let $M_t\in\Delta_H$ be the online parameter. We initialize
at
\begin{equation}
\label{eq:alg-init}
...
\(L\)-Lipschitz. OMD on $\Delta_H$, initialized as in
...
$0\le \psi_t^{(i)}\le \rho_t\le 1/(1-\alpha)$. In particular, $M\mapsto
x_t^{\pi(M)}$ is affine.
\end{lemma}
