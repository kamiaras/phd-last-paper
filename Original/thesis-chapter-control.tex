\chapter{Online Control with Energy Harvesting Constraints}

\label{Section:ChapControl}

\BlankFootnote{The material in this chapter was presented at the 46th Southern California Control Workshop.}

% \begin{abstract}
% This chapter addresses \emph{Online Control} problems in which control actions are constrained by limited energy resources.  
% It generalizes the setting of Chapter~\ref{Section:ChapEnergyharvest}, where the primary application was optimal power control for an energy-harvesting device with a finite-capacity battery.  
% The objective is to minimize a long-term average loss while ensuring that the total energy used never exceeds the available supply.  

% Unlike the previous setup, where the harvested energy arrivals were assumed to be i.i.d., here the arrivals are arbitrary and unknown.  
% Moreover, the loss functions are revealed only after each decision, and the controller has access solely to past observations.  
% We develop a new algorithm within the online control framework that handles this adversarial, history-dependent environment.  
% A further generalization allows the loss to depend not only on the control action but also on the current battery level.  
% The proposed algorithm employs a linear filter whose decision vector lies on a probability simplex, enabling efficient and adaptive control under energy constraints.
% \end{abstract}
...
In this chapter, we generalize the energy-harvesting model studied in the previous chapter along three main dimensions. First, we relax the i.i.d.\ assumption on the energy arrivals and allow an arbitrary (possibly adversarial) sequence of arrivals. Second, while the loss function remains arbitrary as before, it is now allowed to depend not only on the allocation vector but also on the current battery state. Third, we assume that the next energy arrival is unknown at the time the allocation is chosen. 

We begin by analyzing the case where the allocation is a one-dimensional scalar and then extend our results to vector-valued allocations. Conceptually, the framework here is different from that of the previous chapter: instead of competing with a single fixed allocation (as in standard online convex optimization), we compete with a fixed \emph{policy} that can adapt its allocation over time. This policy is drawn from a class that we introduce, called \emph{Simplex Disturbance-Action Control (SDAC)}, which generalizes the Disturbance-Action Control (DAC) framework \cite{hazan_introduction_2025}. While DAC is designed for general linear control problems, our SDAC class is tailored to energy-harvesting systems and naturally enforces energy conservation and energy-availability constraints.
...
A recent line of work in online learning has focused on \emph{online control}, where a dynamical system must be steered under adversarially chosen disturbances and cost functions. Unlike classical stochastic or robust control, the goal is not stabilization or worst-case analysis, but rather to achieve low regret relative to the best policy in hindsight. This viewpoint was initiated in a sequence of works that developed algorithms for linear dynamical systems and established regret bounds under various assumptions on the noise and system structure \cite{agarwal2019online, agarwal2019logarithmic, hazan2020nonstochasticBandit, simchowitz2020improper}. We refer the reader to \cite{hazan_introduction_2025} for a comprehensive review of the area.

A central concept in this literature is the use of DAC policies, which parameterize the control action as an affine or linear function of past disturbances. These policies provide a tractable surrogate for dynamic regret minimization and lead to efficient algorithms even in adversarial environments. Various extensions have been proposed, including handling partial observability \cite{lale2020logarithmic}, and bandit feedback setting \cite{hazan2020nonstochasticBandit}.

Our work can be viewed as further extending this framework to constrained energy-harvesting systems. These constraints fundamentally alter the structure of feasible policies. To address this, we introduce the \emph{Simplex Disturbance-Action Control (SDAC)} class, a natural generalization of DAC that enforces energy conservation and availability in a way tailored to our problem. This connects online control with energy-harvesting models and provides a new policy class suitable for regret minimization under resource constraints.

\subsubsection{Energy Harvesting and Battery Control.}

Online control problems involving energy-harvesting batteries have been extensively studied across several research communities.  
Early work in networking applied Lyapunov optimization techniques to address energy storage constraints.  
For example, Huang and Neely~\cite{Huang2013} developed a near-optimal scheduling policy for energy-harvesting networks with finite-capacity batteries.  
More recently, Yu and Neely~\cite{Yu2019} combined online gradient methods with a drift-plus-penalty framework, yielding power-control algorithms that achieve time-average utility within $O(\epsilon)$ of the optimal value using a battery of size $O(1/\epsilon)$.  
Asgari and Neely~\cite{Asgari2020} further improved these results by introducing a Bregman-divergence-based online method that attains $O(\sqrt{T \ln n})$ regret, thereby reducing the dependence on the decision dimension $n$ to polylogarithmic.  

In parallel, energy usage with battery and inventory constraints has also been analyzed from a competitive analysis perspective.  
Chau et al.~\cite{Chau2016} designed online algorithms for energy storage management that achieve near-optimal competitive ratios under worst-case cost scenarios.  
Yang et al.~\cite{Yang2020} studied a general model of \emph{online linear optimization with inventory management}, a formulation that includes battery control as a special case, and proposed algorithms with provably minimal competitive ratios.  
These studies highlight the intrinsic challenge posed by the temporal coupling of decisions through the battery dynamics, and they collectively provide diverse algorithmic tools—ranging from drift-based online learning to competitive scheduling—for power control in adversarial environments.  

The work of Yu and Neely~\cite{Yu2019}, reviewed in the previous section, considers online convex optimization (OCO) with energy harvesting but incurs an $O(\sqrt{n})$ dependence on system dimension.  
Energy-harvesting control has also been studied outside the OCO framework; see, for instance,~\cite{yang2011optimal,tutuncuoglu2012optimum,blasco2013learning,michelusi2013transmission,shaviv2016universally,wu2016online,arafa2017energy,Huang2013,leonidas-energy-harvest}.  

Related research has explored \emph{inventory constraints} more broadly, including~\cite{yang2020online,lin2019competitive,chau2016cost,neely-inventory-control-cdc2010}.  
For example, Yang et al.~\cite{yang2020online} proposed an algorithm with theoretical guarantees for limited-capacity inventory problems, where the objective function is linear and known at each slot~$t$.  
In contrast, the setting of this work allows for nonlinear and unknown objectives revealed only after decisions are made.  
Additionally, related formulations such as the one-way trading and online knapsack problems~\cite{cao2020optimal,el2001optimal} provide algorithmic insights applicable to constrained OCO, offering useful perspectives for online energy management under uncertainty.

\subsection{Our Contribution}
The concept of using a linear filter defined over a probability simplex has previously appeared in the context of queueing networks~\cite{1354603}, where a fixed weight vector \(w^*\) is employed and shown to ensure finite delay. We independently rediscovered this idea while attempting to generalize the DAC framework. The resulting policy class—rooted in this linear filtering perspective—constitutes our main contribution and is naturally suited to the constrained dynamics considered in this work.




\subsection{Problem setup: one-dimensional allocation}


We consider a discrete-time system with slots \(t \in \{1,2,3,\dots\}\).  
The storage (battery) state is denoted by \(b_t \in [0,B]\), where \(B \in (0,\infty]\) is the capacity (with \(B=\infty\) corresponding to an unbounded battery).  
...
\end{equation}
Arrivals satisfy \(a_t \in [0,1]\) (normalized without loss of generality).  
The control variable \(u_t\) (usage/discharge) is chosen by the algorithm.
...
After the usage decision, a loss function \(c_t : [0,B]^2 \to \mathbb{R}\) is revealed, and the incurred loss is \(c_t(b_t,u_t)\).  
...
\subsubsection{Information Pattern (Adversarial Setting)}
We adopt a online control model in which both the arrival sequence \(\{a_t\}_{t=1}^T\) and the loss sequence \(\{c_t\}_{t=1}^T\) may be adversarially chosen.  
At each time \(t\):
\begin{enumerate}
...
\subsection{No-Wasted-Energy Assumption and Its Consequences}
We assume there exists at least one comparator policy \(\pi \in \Pi\) such that, for all \(t\),
\[
b_t^\pi - u_t^\pi \;\le\; B - 1 .
\]
Since arrivals satisfy \(a_t \le 1\), this condition ensures no energy is lost due to battery saturation.  
...
\subsection{Queueing Network Application}

The same dynamics arise naturally in queueing systems.  
Consider a queue \(q_t \,(\equiv b_t)\) with capacity \(Q \,(\equiv B)\), arrivals \(a_t \in [0,1]\), and service decisions \(s_t \,(\equiv u_t)\).  
The evolution of the queue is identical to \eqref{eq:battery-evo}:
\begin{equation*}
q_{t+1} \;=\; \min\{\,Q,\; q_t - s_t + a_t\,\}.
\end{equation*}
...
The objective is to minimize the long-run average of these costs, i.e., the sum of the average queue length and the average service penalty.


\section{Disturbance Action Control (DAC) Policies}
\label{sec:dac}

The DAC family parameterizes controls as \emph{causal linear functions of the realized past disturbances (arrivals)} together with a state-feedback term. Fix a memory horizon \(H\ge 1\) and a feedback gain \(\kappa\in[0,1]\). A policy in the class \(\mathrm{DAC}_H^\kappa\) is indexed by
\[
...
\end{equation}
where the comparator policy \(\pi\) is chosen once at \(t=1\) and then followed unchanged for all \(t=1,\dots,T\). Throughout, we adopt the no-wasted-energy assumption that the state dynamics simplify to
\begin{equation}
...
This linearization ensures that the overall loss remains convex in the decision variables, enabling the use of efficient OCO algorithms with well-understood regret guarantees.

However, DAC becomes less suitable for the setting considered in this chapter.  
The key limitation is that DAC does not naturally enforce the resource (energy or battery) constraints inherent in our problem.  
...
Unlike DAC, SDAC maintains tractability without an explosion in the number of constraints, enabling efficient and theoretically sound online control under energy limitations.



\section{A New Class of Policies: Simplex-DAC}

We introduce a comparator class \(\mathrm{SDAC}_H^\kappa\), inspired by DAC but tailored to the storage problem.  
...
= b_t^{\pi(w)}.
\]
Thus the usage never exceeds the available battery state.
\end{proof}



Next, we define a surrogate loss: the cost at time \(t\) incurred by choosing \(\pi(w)\) from time \(1\) onward.

\begin{Thm:Lemma}[Convexity and Lipschitz continuity of the surrogate loss]
...
At each round \(t\), the controller selects a weight vector \(w_t \in \Delta_H \coloneqq \{w \in \mathbb{R}^H_{\ge 0} : \sum_{i=1}^H w^{(i)}=1\}\), computes the control law from Eq.~\eqref{eq:u-of-efficient}, and plays the minimum of that value and the available energy in the battery.  
...
\For{$t = 1$ to $T$}
    ...
    \State \textbf{Construct surrogate loss:}
    \[
...
The \emph{Second term} is handled using standard Online Mirror Descent (OMD) guarantees applied to the convex surrogate losses \(\ell_t(w)\) (Lemma~\ref{lem:surrogate-props}).  
...
From Lemma~\ref{lem:surrogate-props}, the surrogate loss is convex and \(L\)-Lipschitz with respect to the \(\ell_1\)-norm, hence \(\|g_t\|_\infty \le L\).  
...
\label{thm:regret}
Suppose the losses \(c_t\) are convex and satisfy Assumption~\ref{eq:Lip}:
\begin{equation*}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/control/arrival.png}
...
the battery dynamics, usage decisions, and incurred costs for both 
$\kappa = 0.1$ and $\kappa = 0.4$.  
For each value of $\kappa$, we show:

\begin{itemize}
    \item Battery level trajectories.
...
\subsubsection{$\kappa = 0.1$ Results: Battery Level Trajectories}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/b-0-200-linears.png}
    \caption{Battery levels for the linear controllers ($K=0.53$ and $K=0.20$) versus
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/b-0-200-egpc.png}
    \caption{Battery levels for EGPC (two step sizes) versus the offline-optimal policy
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/b-700-egpc.png}
    \caption{Battery levels for EGPC (two step sizes) versus the offline-optimal policy
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/b-1000-egpc.png}
    \caption{Battery levels for EGPC (two step sizes) versus the offline-optimal policy
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/c-0-200-linears.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/c-0-200-egpc.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/c-700-egpc.png}
    \caption{Instantaneous costs for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [500,700]$, with $\kappa = 0.1$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/c-1000-egpc.png}
    \caption{Instantaneous costs for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [800,1000]$, with $\kappa = 0.1$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/u-0-200-linears.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/u-0-200-egpc.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/u-700-egpc.png}
    \caption{Usage $u_t$ for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [500,700]$, with $\kappa = 0.1$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.1/u-1000-egpc.png}
    \caption{Usage $u_t$ for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [800,1000]$, with $\kappa = 0.1$.
...
\subsubsection{$\kappa = 0.4$ Results: Battery Level Trajectories}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/b-0-200-linears.png}
    \caption{Battery levels for the linear controllers ($K=0.53$ and $K=0.20$) versus
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/b-0-200-egpc.png}
    \caption{Battery levels for EGPC (two step sizes) versus the offline-optimal policy
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/b-700-egpc.png}
    \caption{Battery levels for EGPC (two step sizes) versus the offline-optimal policy
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/b-1000-egpc.png}
    \caption{Battery levels for EGPC (two step sizes) versus the offline-optimal policy
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/c-0-200-linears.png}
    \caption{Instantaneous costs for the linear controllers versus the offline-optimal policy
    for $t \in [0,200]$, with $\kappa = 0.4$.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/c-0-200-egpc.png}
    \caption{Instantaneous costs for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [0,200]$, with $\kappa = 0.4$.
    This interval shows the initial fluctuation in costs as the controller adapts.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/c-700-egpc.png}
    \caption{Instantaneous costs for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [500,700]$, with $\kappa = 0.4$.
    This figure demonstrates how the cost curves compress as the algorithm approaches
    stable behavior.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/c-1000-egpc.png}
    \caption{Instantaneous costs for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [800,1000]$, with $\kappa = 0.4$.
    By the end of the horizon, the cost trajectories closely track the offline-optimal benchmark.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/u-0-200-linears.png}
    \caption{Usage $u_t$ for the linear controllers versus the offline-optimal policy
    for $t \in [0,200]$, with $\kappa = 0.4$.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/u-0-200-egpc.png}
    \caption{Usage $u_t$ for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [0,200]$, with $\kappa = 0.4$.
    This interval captures the initial adaptation of the usage decisions.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/u-700-egpc.png}
    \caption{Usage $u_t$ for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [500,700]$, with $\kappa = 0.4$.
    The mid-horizon behavior shows reduced variability and improved alignment with the benchmark.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/control/kappa0.4/u-1000-egpc.png}
    \caption{Usage $u_t$ for EGPC (two step sizes) versus the offline-optimal policy
    for $t \in [800,1000]$, with $\kappa = 0.4$.
    This final segment shows the stabilization of usage decisions around the offline-optimal policy.}
\end{figure}
...
\]
whose \(i\)-th component \(x_t^{(i)}\) denotes the energy allocated to sink (task) \(i \in \{1,\ldots,n\}\).  
The system then incurs a loss
\[
c_t(b_t, x_t),
\]
...
The total energy used in round \(t\) equals the \(\ell_1\)-norm of the allocation vector,
\[
u_t \;=\; \|x_t\|_1 \;=\; \sum_{i=1}^n x_t^{(i)},
\]
and the battery evolves according to
\begin{equation}
...
This formulation enables energy to be distributed among multiple tasks while preserving the linear battery dynamics.  
...
\For{$t = 1$ to $T$}
    ...
    \State \textbf{Construct and evaluate surrogate loss:}
    \[
        \ell_t(W)
        \coloneqq
...
i\,\eta L,
\]
where \(L\) is the Lipschitz constant of the surrogate losses \(\ell_t\) (from Lemma~\ref{lem:surrogate-props-W}).  
\end{Thm:Corollary}
...
\begin{Thm:Theorem}[Regret bound]
\label{thm:regret-W}
Suppose the losses \(c_t\) are convex and \(L\)-Lipschitz (with \(L = 2 L_x + \tfrac{L_b}{\kappa}\)).  
...
This chapter extended the energy-harvesting framework of Chapter~\ref{Section:ChapEnergyharvest} to an online control setting with adversarial, history-dependent dynamics and battery-dependent costs. We introduced a class of Simplex-DAC policies, \(\mathrm{SDAC}_H^\kappa\), that capture a rich family of linear-filter controllers while preserving feasibility with respect to the battery constraints. Building on this parametrization, we designed the Exponentiated-Gradient Perturbation Controller (EGPC), which updates the filter weights via online convex optimization on a surrogate loss constructed from observed trajectories. We proved that EGPC achieves sublinear regret with respect to the best SDAC policy in hindsight while always respecting energy availability. These results show that one can obtain robust, data-driven control policies for energy-harvesting systems without stochastic assumptions on the arrivals or losses, and they lay the groundwork for future extensions to richer controller classes and partially observed state dynamics.
